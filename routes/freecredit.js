  ...
  try {
    ...
    if (user.wallet > 1) {
      return res.status(200).json({
        success: false,
        message: {
          en: "Wallet balance must be less than $1",
          zh: "钱包余额必须少于1元",
          ms: "Baki dompet mesti kurang daripada $1",
          zh_hk: "錢包餘額必須少於1元",
          id: "Saldo dompet harus kurang dari $1",
        },
      });
    }

    if (user.duplicateBank === true) {
      return res.status(200).json({
        success: false,
        message: {
          en: "You are not eligible for this free credit promotion",
          zh: "您不符合此免费注册彩金的资格",
          ms: "Anda tidak layak untuk promosi kredit percuma ini",
          zh_hk: "您不符合此免費註冊彩金嘅資格",
          id: "Anda tidak memenuhi syarat untuk promosi kredit gratis ini",
        },
      });
    }

    const nameCheck = await checkForSimilarNames(userId, user.fullname);
