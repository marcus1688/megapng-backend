      ...
      if (req.body.depositAmount < minDeposit) {
        return res.status(200).json({
          ...
          message: {
            ...
            zh_hk: `最低存款金額為 ${minDeposit}`,
            ms: `Jumlah deposit minimum adalah ${minDeposit}`,
            id: `Jumlah deposit minimum adalah ${minDeposit}`,
          },
        });
      }
      ...
      if (maxDeposit > 0 && req.body.depositAmount > maxDeposit) {
        return res.status(200).json({
          ...
          message: {
            ...
            zh_hk: `最高存款金額為 ${maxDeposit}`,
            ms: `Jumlah deposit maksimum adalah ${maxDeposit}`,
            id: `Jumlah deposit maksimum adalah ${maxDeposit}`,
          },
        });
      }

      const hasPromotion =
        req.body.promotionId || req.body.isLuckySpinClaim === "true";
      ...
      if (hasPromotion) {
        if (user.wallet >= 1) {
          return res.status(200).json({
            success: false,
            message: {
              en: "Wallet balance must be less than 1 to apply for promotion",
              zh: "申请优惠钱包余额必须少于1",
              zh_hk: "申請優惠錢包餘額必須少於1",
              ms: "Baki dompet mestilah kurang dari 1 untuk memohon promosi",
              id: "Saldo dompet harus kurang dari 1 untuk mengajukan promosi",
            },
            bonusRestriction: true,
