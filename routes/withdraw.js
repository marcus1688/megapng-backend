    ...
    if (turnoverCheck.success) {
      ...
      if (turnoverCheck.requiredTurnover) {
        detailedMessage = {
          ...
          zh_hk: `未滿足流水要求。需要: RM${turnoverCheck.requiredTurnover.toFixed(
            2
          )}，當前: RM${turnoverCheck.currentTurnover.toFixed(
            2
          )}，還差: RM${turnoverCheck.remainingTurnover.toFixed(2)}`,
        ...
        detailedMessage = {
          ...
          zh_hk: `未滿足餘額要求。需要餘額: RM${turnoverCheck.requiredAmount.toFixed(
            2
          )}，當前: RM${turnoverCheck.currentBalance.toFixed(
            2
          )}，缺少: RM${turnoverCheck.remainingAmount.toFixed(2)}`,
...
router.post("/api/withdraw", authenticateToken, async (req, res) => {
  try {
    ...
    if (withdrawAmount < minWithdraw) {
      return res.status(200).json({
        ...
        message: {
          ...
          zh_hk: `最低提款金額為 ${minWithdraw}`,
          ms: `Jumlah pengeluaran minimum adalah ${minWithdraw}`,
          id: `Jumlah penarikan minimum adalah ${minWithdraw}`,
        },
      });
    }
    ...
    if (maxWithdraw > 0 && withdrawAmount > maxWithdraw) {
      return res.status(200).json({
        ...
        message: {
          ...
          zh_hk: `最高提款金額為 ${maxWithdraw}`,
          ms: `Jumlah pengeluaran maksimum adalah ${maxWithdraw}`,
          id: `Jumlah penarikan maksimum adalah ${maxWithdraw}`,
        },
      });
    }
    ...
    let hasActiveBonusRestriction = false;
    ...
    if (
      ...
      hasActiveBonusRestriction = false;
    } else {
      let isDepositLatest =
        latestDeposit &&
        (!latestBonus || latestDeposit.createdAt > latestBonus.createdAt);
      ...
      if (isBonusLatest) {
        hasActiveBonusRestriction = true;
      } else if (isDepositLatest) {
        const relatedBonus = await Bonus.findOne({
          depositId: latestDeposit._id,
          status: "approved",
    ...
    if (hasActiveBonusRestriction) {
      const remainingBalance = user.wallet - withdrawAmount;
      if (remainingBalance >= 1) {
        return res.status(200).json({
          success: false,
          message: {
            en: "Bonus transaction detected, please withdraw all wallet balance",
            zh: "检测到此交易涉有奖金，请提款所有钱包余额",
            zh_hk: "檢測到呢個交易涉及獎金，麻煩提取晒所有錢包餘額",
            ms: "Transaksi bonus dikesan, sila keluarkan semua baki dompet",
            id: "Transaksi bonus terdeteksi, silakan tarik semua saldo dompet",
          },
          bonusCapRestriction: true,
          currentBalance: user.wallet,
          withdrawAmount: withdrawAmount,
          remainingBalance: remainingBalance,
          maxAllowedWithdraw: Math.floor((user.wallet - 0.99) * 100) / 100,
        });
      }
    }
